...
def editprofile(request):
    ...
    try:
        forminstance = db.child("Users").child("teacher").child(uid).get().val()
        print(forminstance)
        form = StaffForm(forminstance)
    except RuntimeError:
        pass
...
def takequiz(request,uid,quizid):
    question_list = getrelation("Quiz",quizid,"Questions","question")
    return render(request, "source/takequiz.html",{'uid':uid, 'question_list':question_list})

# def takequiz(request,uid,cid):
#     quizlist=getrelation("Courses",cid,'Quiz','name')
#     return render(request, "source/takequiz.html",{'uid':uid,'quizlist':quizlist})
...
def student_course(uid):
   try:
       x=db.child('Users').child('Student').child(uid).child('grade').get().val()
       print(x)
       stdcourse=getrelation("Grade",x,'Courses','title')
       return stdcourse
   except:
       pass
